...
import { getMyAllPoolsHandler } from "../AirDrops/ApiAirdropHandler";
...
const MyAirDropList = () => {
  ...
  const fetchAirdrops = async () => {
    ...
    try {
      setLoading(true);
      const response = await getMyAllPoolsHandler(account);
      const currentTime = new Date();
      console.log("getMyAllPoolsHandler", response?.Airdrop);
      const updatedPoolList = (response?.data || []).map((temp) => {
        const startTime = new Date(temp.Start_Time);
        const TotalAllocation = temp?.TotalAllocation || 0;
        const Total_Claimed = temp?.Total_Claimed || 0;
        const isCancelled = temp.Is_Canceled || false;

        let status = "N/A";
        if (isCancelled) {
          status = "Cancelled";
        } else if (startTime === null || startTime > currentTime) {
          status = "Upcoming";
        } else if (TotalAllocation === Total_Claimed && TotalAllocation !== 0) {
          status = "Ended";
        } else {
          status = "Live";
        }
      ...
      setPoolInfo(updatedPoolList);
      setTotalPages(Math.ceil(updatedPoolList.length / limit));
    } catch (err) {
