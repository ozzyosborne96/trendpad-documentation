...
import { getAllAirDropHandler } from "../AirDrops/ApiAirdropHandler";
...
const AllDrops = () => {
  ...
  const renderCard = (pool, index) => (
    <Grid item xs={12} sm={6} md={4} key={index}>
      ...
      >
        ...
        >
          {(pool?.AirDrop_Tittle?.length > 15
            ? pool.AirDrop_Tittle.slice(0, 15) + "..."
  ...
  const fetchAirdrops = async (currentPage = 1) => {
    try {
      setLoading(true);
      const { chainId } = await getChainInfo();
      const response = await getAllAirDropHandler(limit, currentPage, chainId);
      console.log("response", response?.data);
      const data = response?.data;
      console.log("updatedPoolList", data);
      setPoolInfo(data || []);
      ...
      {loading ? (
        ...
        <>
          <Grid container spacing={4} justifyContent="center">
            {poolInfo.map((pool, index) => renderCard(pool, index))}
          </Grid>
